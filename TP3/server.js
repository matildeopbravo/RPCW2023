...
http.createServer((req,res) => {
    ...
    if(dicURL.pathname == "/") {
        ...
        if (dicURL.query.id) {
            ...
            axios.get("http://localhost:3000/pessoas?_sort=nome&order=asc" + search)
                .then ((resp) =>  {
                    ...
                    res.end(pag)
                    // debugging
                    fs.writeFile('pagina.html',pag, err => {if(err) console.log(err)})
                })
                .catch(erro => {
        ...
        let outro_req = axios.get(
            "http://localhost:3000/pessoas?sexo=outro"
          );

        Promise.all([feminino_req,masculino_req,outro_req])
            .then((resp) => {
                res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8' })
                res.end(mypages.distribuicaoSexo(resp[0].data,resp[1].data,resp[2].data))
            })
            .catch ((err) => {
